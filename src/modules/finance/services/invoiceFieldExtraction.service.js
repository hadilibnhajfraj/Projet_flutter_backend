...
function extractPaymentLinear(text) {
  ...
  const methodMatch = text.match(/\b(TRAITE|CH[ÈE]QUE|ESP[ÈE]CES?|VIREMENT|CARTE\s*BANCAIRE)\b/i);
  const method = methodMatch ? field(methodMatch[1], 0.6) : field(null, 0);
...
const PAYMENT_METHOD_WORDS = /^(TRAITE|CH[ÈE]QUE|ESP[ÈE]CES?|VIREMENT|CARTE\s*BANCAIRE)$/i;
...
function extractPaymentMethodNearLabel(words, labelWord) {
  ...
  return candidate ? field(candidate.text.trim(), 0.85) : field(null, 0);
}
...
function isPositionalLabel(text) {
  ...
  return Boolean(matchColumnKey(normalized, { count: 0 })); // en-têtes du tableau produits
}
...
async function extractInvoiceFieldsPositional(page1Words, { fullText, filePath, engine, pages }) {
  ...
  // des lignes puis sur Total TTC − Total HT si rien n'a pu être reconstruit).
...
async function extractInvoiceFields({ fullText, filePath, engine, pages }) {
  ...
  if (page1Words.length) {
    ...
    if (positional.invoiceNumber.value) return positional;
  }
  return extractInvoiceFieldsLinear({ fullText, filePath, engine, pages });
}
